...
plot_freq_date_group <- function(x, group, title = TRUE) {
  ...
  factor_levels <- list(
    ...
    'weekday' = format(seq(from = as.Date('2018-04-02'), 
  ...
  if(nrow(plot_data[is.na(count)]) > 0) {
    plot <- plot + 
      geom_point(data = plot_data[is.na(count)], mapping = aes(x = date, y = 0),
                 color = 'red')
  }
  plot <- plot + 
  ...
  if(title) {
    plot <- plot + labs(title = 'Total number of records by group. Group without data are plotted in red.', 
...
plot_missing_date <- function(x) {
  ...
  if(yearly) {
    ...
    if(nrow(plot_data[is.na(count)]) > 0) {
      plot <- plot + 
        geom_point(data = plot_data[is.na(count)][date > min_date & date < max_date], 
    ...
    plot <- plot + 
      coord_flip() + 
      labs(x = 'Number of recods', y = 'Month', 
    ...
    levels_dow <- format(seq(from = as.Date('2018-01-01'), 
...
desc_date <- function(x, plot = TRUE) {
  ...
  summary_output <- data.frame(
    Indicator = c('Length', 'Number of NAs', 'Number of unique values', 'Minimum date', 
                  'Maximum date'),
  ...
  invisible(list(summary_stat = summary_output,
                 freq_plot = freq_plot,
