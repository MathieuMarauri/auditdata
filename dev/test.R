...
setDT(diamonds)
diamonds[, date := seq(from = as.Date('2012-10-08'), to = as.Date('2018-10-08'),
                       by = 'day')]
...
dataQuality <- function(data, numeric_cutoff = -1, na_type = c('', ' ', 'NA', 'NULL'), 
  ...
  if(!global_only) {
    ...
    if(length(date_var) > 0) {
      date_output <- dateQuality(data = data, 
                                 date_var = date_var, 
                                 max_length = max_length)
...
#'
#' @export
reportDataQuality <- function(data = NULL,
                              quality_res = NULL,
                              file = NULL,
                              numeric_cutoff = -1,
                              na_type = c('', ' ', 'NA', 'NULL'),
  ...
  if(!is.null(data) & (is.null(quality_res) | (!is.null(quality_res) & class(quality_res)[2] != 'qualityResult')))
    quality_res <- dataQuality(data = data, 
                               numeric_cutoff = numeric_cutoff, 
                               na_type = na_type, 
                               max_length = max_length, 
  ...
  if(!global_only){
    ...
    if('categorical' %in% names(quality_res)){
      ...
      liste_names <- sapply(X = names(output_character), 
    ...
    if('date' %in% names(quality_res)){
      ...
      # date output
      liste_names <- sapply(X = names(output_date_freq), 
...
test <- reportDataQuality(diamonds)
