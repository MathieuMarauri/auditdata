...
plotHitogram <- function(x, bins = 50, theme = NULL) {
  ggplot(data = NULL, mapping = aes(x = diamonds$depth)) + 
    geom_histogram(bins = 50) + 
    theme
}
plotHitogram(diamonds$depth)


plotNumeric <- function(data, numeric_var, theme = NULL) {
  lapply(X = numeric_var, 
         FUN = function(name) ggplot(data = data, mapping = aes_string(x = name)) + 
           )
}
